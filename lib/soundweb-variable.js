module.exports = function(RED) {
    ...
    function LevelControl(config) {
        ...
        this.server.on('rxData', (rx) => {
            if (Buffer.compare(rx.address, address) == 0 && Buffer.compare(rx.cmd_id, cmd_id) == 0) {
                ...
                msg.payload = this.server.bssLib.bufferToInt(rx.data);
                this.send(msg);
            }
        });
        ...
        this.on('input', (msg) => {
            if (typeof msg.payload == 'number' && msg.payload >= 0) {
                ...
                var data = this.server.bssLib.intToBuffer(msg.payload);
    ...
    RED.nodes.registerType("soundweb-variable", LevelControl);
}